...
export const addFileInfo = (request, response) => {
  const { filename, filesize, upload_path, storage, signedUrl, body } =
    request.body;
    ...
    client.query(query, values, (error, result) => {
      ...
      response.status(200).send({
        ...
        data: {
          ...
          download_code: `/${storage}/downloadFile/${download_code}`,
          code: download_code,
          signedUrl: signedUrl,
          body: body,
        },
      ...
      if (result.rows[0].storage == "S3") {
        ...
        response.status(200).send({
          ...
          data: {
            ...
            downloadPath: result.rows[0].upload_path,
...
export const getStorage = async (request, response) => {
  pool.connect((err, client, done) => {
    const query = `SELECT storage FROM files WHERE download_code='${request.params.download_code}'`;
    client.query(query, (error, result) => {
      done();
      if (error) {
        console.log(error);
      ...
      response.status(200).send({
        status: "Success",
        data: { downloadCode: result.rows[0].storage },
      });
    });
  });
};
